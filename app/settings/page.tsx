...
async function fetchAlpacaStatus(): Promise<{ connected: boolean; account_id: string | null; status_message: string }> {
  const res = await apiFetch('/api/user/alpaca/status');
  if (!res.ok) throw new Error('Failed to load Alpaca status');
  return res.json() as Promise<{ connected: boolean; account_id: string | null; status_message: string }>;
}

async function connectAlpaca(apiKey: string, secretKey: string): Promise<{ connected: boolean; account_id: string | null; status_message: string }> {
  ...
  if (!res.ok) {
    ...
    throw new Error(err.detail ?? 'Failed to connect Alpaca account');
  }
  return res.json() as Promise<{ connected: boolean; account_id: string | null; status_message: string }>;
}
...
async function disconnectAlpaca(): Promise<void> {
  ...
  if (!res.ok) throw new Error('Failed to disconnect Alpaca account');
}
...
function AlpacaCard() {
  ...
  const connectMutation = useMutation({
    ...
    onSuccess: () => {
      setApiKey('');
      setSecretKey('');
      void queryClient.invalidateQueries({ queryKey: ['alpaca-status'] });
    },
  });

  const disconnectMutation = useMutation({
    mutationFn: disconnectAlpaca,
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ['alpaca-status'] }),
  });
        ...
        {isLoading ? (
          ...
          <div className="space-y-4">
            ...
            {disconnectMutation.isError && (
              <p className="text-sm text-destructive">{(disconnectMutation.error as Error).message}</p>
            )}
          </div>
        ) : (
          <div className="space-y-4">
